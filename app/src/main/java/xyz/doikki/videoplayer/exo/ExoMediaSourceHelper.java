...
public final class ExoMediaSourceHelper {
    ...
    public MediaSource getMediaSource(String uri, Map<String, String> headers, boolean isCache, String mimeType) {
        ...
        // 构建MediaItem，优先使用传入的MIME类型
        MediaItem.Builder builder = MediaItem.Builder().setUri(contentUri);
        if (mimeType != null && !mimeType.isEmpty()) {
            ...
            switch (contentType) {
                case C.TYPE_DASH:
                    builder.setMimeType(com.google.android.exoplayer2.util.MimeTypes.APPLICATION_MPD);
                    break;
                case C.TYPE_HLS:
                    builder.setMimeType(com.google.android.exoplayer2.util.MimeTypes.APPLICATION_M3U8);
                    break;
                default:
                    // 对于其他类型，不设置MIME类型，让ExoPlayer自动检测
                    break;
            }
        }
