...
public final class ExoMediaSourceHelper {
    ...
    public MediaSource getMediaSource(String uri, Map < String, String > headers, boolean isCache, String mimeType) {
        ...
        if("rtmp".equals(contentUri.getScheme())) {
            return new ProgressiveMediaSource.Factory(new RtmpDataSourceFactory(null)).createMediaSource(MediaItem.fromUri(contentUri));
        ...
        // 构建MediaItem
        MediaItem.Builder builder = new MediaItem.Builder().setUri(contentUri);
...
private int inferContentType(String uri) {
    ...
    private DataSource.Factory getHttpDataSourceFactory() {
        if(mHttpDataSourceFactory == null) {
            mHttpDataSourceFactory = new OkHttpDataSource.Factory(mOkClient).setUserAgent(mUserAgent);
        }
    ...
    private void setHeaders(Map < String, String > headers) {
        if(headers != null && headers.size() > 0) {
            ...
            while(iter.hasNext()) {
                ...
                if(v != null) headers.put(k, v.trim());
            }
