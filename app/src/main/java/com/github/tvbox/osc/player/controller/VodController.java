...
//		simSeekPosition = 0;      //XUAMENG重要
        ...
        if (action == KeyEvent.ACTION_DOWN) {
            if (keyCode == KeyEvent.KEYCODE_DPAD_RIGHT || keyCode == KeyEvent.KEYCODE_DPAD_LEFT) {
                if (isInPlayback) {
					...
					if ((System.currentTimeMillis() - DOUBLE_CLICK_TIME_2) < 300 || isLongClick || isAnimation){                  //xuameng 防播放打断动画					
						return true;
					}
					DOUBLE_CLICK_TIME_2 = System.currentTimeMillis();
					if (isInPlayback) {
    ...
    public boolean onSingleTapConfirmed(MotionEvent e) {            //延时回调,延迟时间是 180 ms,
		if ((System.currentTimeMillis() - DOUBLE_CLICK_TIME_2) < 300 || isAnimation){                  //xuameng 防止180ms内点击返回键，又会弹击菜单				
			return false;
			}
		DOUBLE_CLICK_TIME_2 = System.currentTimeMillis();
