...
import com.github.tvbox.osc.util.ImgUtilSearch;   //xuamengBASE64图片
    ...
    protected void convert(BaseViewHolder helper, Movie.Video item) {
        ...
        if (Hawk.get(HawkConfig.SEARCH_VIEW, 0) == 0) {
            ...
            if (!TextUtils.isEmpty(item.pic)) {
                if(ImgUtilSearch.isBase64Image(item.pic)){
                    // xuameng 如果是 Base64 图片，解码并设置
                    ivThumb.setImageBitmap(
                        ImgUtilSearch.decodeBase64ToRoundBitmap(item.pic, radius)   //xuameng 用这个方法进行圆角设置
                    );
                }else {
                    Picasso.get()
                            .load(item.pic)
                            .transform(new RoundTransformation(MD5.string2MD5(item.pic))
                                    .centerCorp(true)
                                    .override(AutoSizeUtils.mm2px(mContext, ImgUtilSearch.defaultWidth), AutoSizeUtils.mm2px(mContext, ImgUtilSearch.defaultHeight))
                                    .roundRadius(AutoSizeUtils.mm2px(mContext, 10), RoundTransformation.RoundType.ALL))
                            .placeholder(R.drawable.img_loading_placeholder)
                        .noFade()
                        //  .error(R.drawable.img_loading_placeholder)
						    .error(ImgUtilSearch.createTextDrawable(item.name))
             ...
             //   ivThumb.setImageResource(R.drawable.img_loading_placeholder);
				ivThumb.setImageDrawable(ImgUtilSearch.createTextDrawable(item.name));
            }
        }
    }
}
