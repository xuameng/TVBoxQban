...
public class OkHttp {
    ...
    public static synchronized OkHttpClient client() {
        ...
        if (base != null)
            return client = base.newBuilder().dns(dns()).addInterceptor(defaultHeaders()).build();
    ...
    public static String string(String url) {
        ...
        try (Response res = newCallResp(url)) {
    ...
    public static String string(String url, long timeout) {
        if (url == null || !url.startsWith("http")) return "";
        try (Response res = newCallResp(client(timeout), url)) {
            return res.body() != null ? res.body().string() : "";
        } catch (Exception e) {
            e.printStackTrace();
            return "";
    ...
    public static String string(String url, Map<String, String> headers) {
        if (url == null || !url.startsWith("http")) return "";
        try (Response res = newCallResp(url, headers)) {
            return res.body() != null ? res.body().string() : "";
        } catch (Exception e) {
            e.printStackTrace();
            return "";
    ...
    public static Response newCallDownload(String url, Map<String, String> headersMap) throws IOException {
        return newCallResp(url, headersMap);
    }
